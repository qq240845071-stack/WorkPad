...
function hasTencentAsrConfig() {
  return hasTencentAsrDirectConfig() || hasTencentAsrProxyConfig();
}

function hasTencentAsrDirectConfig() {
  return Boolean(cleanText(process.env.TENCENT_ASR_SECRET_ID) && cleanText(process.env.TENCENT_ASR_SECRET_KEY));
}

function getTencentAsrProxyConfig() {
  return {
    baseUrl: cleanText(process.env.TENCENT_ASR_PROXY_BASE_URL || process.env.WECOM_PROXY_BASE_URL || process.env.WECOM_PROXY_URL).replace(/\/+$/, ""),
    secret: cleanText(process.env.TENCENT_ASR_PROXY_SECRET || process.env.WECOM_PROXY_SECRET),
  };
}

function hasTencentAsrProxyConfig() {
  const { baseUrl, secret } = getTencentAsrProxyConfig();
...
async function createTencentAsrTranscription({ audioBuffer, filename = "voice.amr", contentType = "application/octet-stream" }) {
  if (hasTencentAsrProxyConfig() && process.env.TENCENT_ASR_PROXY_DISABLED !== "true") {
    return createTencentAsrProxyTranscription({ audioBuffer, filename, contentType });
  }
  return createTencentAsrDirectTranscription({ audioBuffer, filename, contentType });
}

async function createTencentAsrProxyTranscription({ audioBuffer, filename = "voice.amr", contentType = "application/octet-stream" }) {
  if (!Buffer.isBuffer(audioBuffer) || !audioBuffer.length) {
    throw new Error("腾讯云 ASR 代理没有收到可识别的语音内容。");
  }
  const { baseUrl, secret } = getTencentAsrProxyConfig();
  if (!baseUrl || !secret) {
    throw new Error("腾讯云 ASR 代理还没有配置地址或密钥。");
  }

  const response = await fetch(`${baseUrl}/asr-sentence`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok || payload.error || payload.ok === false) {
    throw new Error(payload.error || payload.message || `腾讯云 ASR 代理请求失败：${response.status}`);
  }
  const text = cleanText(payload.text || payload.result);
  if (!text) throw new Error("腾讯云 ASR 代理没有返回可用文本。");
  return {
    text,
    model: cleanText(payload.model, cleanText(process.env.TENCENT_ASR_ENGINE, "16k_zh")),
    providerName: cleanText(payload.providerName, "腾讯云 ASR 一句话识别（北京代理）"),
    task: "transcription",
    usage: payload.usage || null,
  };
}

async function createTencentAsrDirectTranscription({ audioBuffer, filename = "voice.amr", contentType = "application/octet-stream" }) {
  if (!hasTencentAsrDirectConfig()) {
...
module.exports = {
  ...
  hasTencentAsrConfig,
  hasTencentAsrDirectConfig,
  hasTencentAsrProxyConfig,
};
